...
using src.IOC.Exception;
using src.IOC.Service;
using src.Log;
using static src.Log.Logs.LogColors;
    ...
    public class Compositor
    {
        ...
        private bool ResolveDependencies()
        {
            foreach (KeyValuePair<Type, List<FieldEntry>> slotsForType in m_dependencySlots)
            {
                ...
                {
                    Logs.Log($"Service type '{type}' is missing from the instantiated services!", Error, Red);
                    return false;
                }
        ...
        private async UniTaskVoid InitCompositor()
        {
            ...
            if (!composed)
            {
                Logs.Log($"Composition failed to sort dependencies - systems won't be started!", Error, Red);
            }
        }
